...
def test_load_existing_jsonl_with_data():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(memories_path, "w") as f:
            ...
            mem2 = Memory(id="existing-2", content="Existing memory 2", 
...
def test_delete_relation(temp_storage):
    ...
    relation = Relation(id="rel-1", from_memory_id="mem-1", 
                       to_memory_id="mem-2", relation_type="references")
    temp_storage.create_relation(relation)
...
def test_load_relations_with_deletion_markers():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(relations_path, "w") as f:
            rel1 = Relation(id="rel-1", from_memory_id="m1", 
                          to_memory_id="m2", relation_type="references")
            rel2 = Relation(id="rel-2", from_memory_id="m2", 
                          to_memory_id="m3", relation_type="similar_to")
...
def test_search_with_window_days(temp_storage):
    ...
    recent_mem = Memory(id="recent", content="Recent memory", last_used=now - 2*86400)  # 2 days ago
...
def test_batch_save_memories(temp_storage):
    ...
    memories = [
        Memory(id=f"batch-{i}", content=f"Batch memory {i}")
        for i in range(5)
    ]
...
def test_data_persists_across_connections():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        mem = Memory(id="persist-1", content="Persistent memory",
                    meta=MemoryMetadata(tags=["persist", "test"]))
        storage1.save_memory(mem)
        
        rel = Relation(id="rel-persist", from_memory_id="persist-1",
                      to_memory_id="persist-1", relation_type="self")
...
def test_list_memories_with_limit_offset(temp_storage):
    ...
    now = int(time.time())
    for i in range(10):
        mem = Memory(id=f"page-{i}", content=f"Memory {i}", 
                    last_used=now - i*100)  # Decreasing timestamps
        temp_storage.save_memory(mem)
...
def test_get_all_embeddings(temp_storage):
    ...
    mem1 = Memory(id="m1", content="With embedding", 
                 embed=[0.1, 0.2, 0.3], status=MemoryStatus.ACTIVE)
    mem2 = Memory(id="m2", content="No embedding", status=MemoryStatus.ACTIVE)
    mem3 = Memory(id="m3", content="Promoted with embedding",
                 embed=[0.4, 0.5, 0.6], status=MemoryStatus.PROMOTED)
...
def test_operations_require_connection_all_methods():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with pytest.raises(RuntimeError, match="Storage not connected"):
            storage.create_relation(Relation(id="r1", from_memory_id="m1", 
                                            to_memory_id="m2", relation_type="test"))
...
def test_save_memories_batch_updates_tag_index(temp_storage):
    ...
    memories = [
        Memory(id=f"batch-tag-{i}", content=f"Memory {i}", 
               meta=MemoryMetadata(tags=[f"tag{i}"]))
        for i in range(3)
    ]
